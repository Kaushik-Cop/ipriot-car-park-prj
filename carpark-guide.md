...
|--------------|---------------------------------------------|----------------------------------------------------------|
| `CarPark`    | bays , license_plate, capacity, temparature | get_available_bays(), update_temperature(temp: float)    |
| `Sensor`     | occupied                                    | get_occupancy()                                          |
| `Display`    | dispaly_id, current_temparature             | update_display(available_bays: int, temperature: float)  |
...
> Carpark because car park knows the capacity of cars
>Q. Which class is responsible for the current temperature (and why)?
> Sensor class measures the current temparature
>Q. Which class is responsible for the time (and why)?
> None as time is based on internal time on the pc
--------
...
> The register method should be tested in the CarPark unit tests because it is the car park’s responsibility to manage its components.
> Sensor and Display tests should focus on their own behavior, while the CarPark tests ensure that only valid components are accepted and invalid ones raise a TypeError.
